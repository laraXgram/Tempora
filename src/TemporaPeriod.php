...
use LaraGram\Tempora\Exceptions\EndLessPeriodException;
use LaraGram\Tempora\Exceptions\InvalidCastException;
use LaraGram\Tempora\Exceptions\InvalidIntervalException;
use LaraGram\Tempora\Exceptions\InvalidPeriodDateException;
use LaraGram\Tempora\Exceptions\InvalidPeriodParameterException;
use LaraGram\Tempora\Exceptions\NotATemporaClassException;
use LaraGram\Tempora\Exceptions\NotAPeriodException;
use LaraGram\Tempora\Exceptions\UnknownGetterException;
use LaraGram\Tempora\Exceptions\UnknownMethodException;
use LaraGram\Tempora\Exceptions\UnreachableException;
use LaraGram\Tempora\Traits\DeprecatedPeriodProperties;
...
require PHP_VERSION < 8.2
    ? __DIR__.'/../lazy/Tempora/ProtectedDatePeriod.php'
    : __DIR__.'/../lazy/Tempora/UnprotectedDatePeriod.php';
// @codeCoverageIgnoreEnd

/**
 * Substitution of DatePeriod with some modifications and many more features.
 *
 * @method static static|TemporaInterface start($date = null, $inclusive = null) Create instance specifying start date or modify the start date if called on an instance.
 ...
 * @method static static|TemporaInterface end($date = null, $inclusive = null) Create instance specifying end date or modify the end date if called on an instance.
 ...
 * @method static static|TemporaInterval interval($interval = null) Create instance with given date interval or modify the interval if called on an instance.
 ...
 */
class TemporaPeriod extends DatePeriodBase implements Countable, JsonSerializable
{
    use LocalFactory;
    ...
    /**
     * Yield TemporaImmutable instances.
     *
     * @var int
     */
     ...
     * @var string
     */
    protected const DEFAULT_DATE_CLASS = Tempora::class;
     ...
     */
    protected string $dateClass = Tempora::class;

    /**
     * Underlying date interval instance. Always present, one day by default.
     */
    protected ?TemporaInterval $dateInterval = null;
     ...
     */
    protected int|float|null $temporaRecurrences = null;
     ...
     * Equal to null only before the first iteration.
     */
    protected ?TemporaInterface $temporaCurrent = null;
    ...
    /**
     * Make a TemporaPeriod instance from given variable if possible.
     */
    ...
    /**
     * Create a new instance from a DatePeriod or TemporaPeriod object.
     */
    ...
    /**
     * Create TemporaPeriod from ISO 8601 string.
     */
    public static function createFromIso(string $iso, ?int $options = null): static
    {
        ...
        $instance->options = ($instance instanceof TemporaPeriodImmutable ? static::IMMUTABLE : 0) | $options;
    ...
    private static function makeInterval(mixed $input): ?TemporaInterval
    {
        try {
            return TemporaInterval::make($input);
        } catch (Throwable) {
            return null;
        }
    }

    private static function makeTimezone(mixed $input): ?TemporaTimeZone
    {
        if (!\is_string($input)) {
            return null;
        }
        ...
        try {
            return TemporaTimeZone::create($input);
        } catch (Throwable) {
            return null;
        }
    }
     ...
     * @example
     * ```
     * TemporaPeriod::macro('middle', function () {
     *   return $this->getStartDate()->average($this->getEndDate());
     * });
     * echo TemporaPeriod::since('2011-05-12')->until('2011-06-03')->middle();
     * ```
     *
     ...
     *   }
     * });
     * echo TemporaPeriod::create('2000-01-01', '2000-02-01')->addDays(5)->subDays(3);
     * ```
     *
    ...
    public function __construct(...$arguments)
    {
        ...
        if ($raw === null && isset($sortedArguments['start'])) {
            ...
            $raw = [
                ...
                $sortedArguments['interval'] ?? TemporaInterval::day(),
                $end,
            ];
        }
        ...
        if ($this->dateInterval === null) {
            $this->setDateInterval(TemporaInterval::day());
     ...
     * @param string $name
     *
     * @return bool|TemporaInterface|TemporaInterval|int|null
     */
    ...
    {
        if (!is_a($dateClass, TemporaInterface::class, true)) {
            throw new NotATemporaClassException($dateClass);
        }

        $self = $this->copyIfImmutable();
        $self->dateClass = $dateClass;

        if (is_a($dateClass, Tempora::class, true)) {
            $self->options = $self->options & ~static::IMMUTABLE;
        } elseif (is_a($dateClass, TemporaImmutable::class, true)) {
            $self->options = $self->options | static::IMMUTABLE;
        }
    ...
    public function setDateInterval(mixed $interval, Unit|string|null $unit = null): static
    {
        ...
        if (!$interval = TemporaInterval::make($interval, $unit)) {
    ...
    {
        $self = $this->copyIfImmutable();
        $self->setDateInterval(TemporaInterval::day());
     ...
     */
    public function getDateInterval(): TemporaInterval
    {
     ...
     */
    public function getStartDate(?string $rounding = null): TemporaInterface
    {
     ...
     * @param string|null $rounding Optional rounding 'floor', 'ceil', 'round' using the period interval.
     */
    public function getEndDate(?string $rounding = null): ?TemporaInterface
    {
        if (!$this->endDate) {
            return null;
        }
    ...
    {
        return $this->temporaRecurrences;
    }
     ...
     */
    public function getIncludedStartDate(): TemporaInterface
    {
     ...
     */
    public function getIncludedEndDate(): TemporaInterface
    {
        $end = $this->getEndDate();
    ...
    public function resetFilters(): static
    {
        ...
        if ($self->temporaRecurrences !== null) {
    ...
    public function setRecurrences(int|float|null $recurrences): static
    {
        ...
        /** @var self $self */
        $self = $this->copyIfImmutable();
        $self->temporaRecurrences = $recurrences === INF ? INF : (int) $recurrences;
     ...
     */
    public function current(): ?TemporaInterface
    {
        return $this->valid()
            ? $this->prepareForReturn($this->temporaCurrent)
            : null;
    }
    ...
    {
        $this->key = 0;
        $this->temporaCurrent = ([$this->dateClass, 'make'])($this->startDate);
        ...
        $this->temporaCurrent->settings($settings);
        $this->timezone = static::intervalHasTime($this->dateInterval) ? $this->temporaCurrent->getTimezone() : null;

        if ($this->timezone) {
            $this->temporaCurrent = $this->temporaCurrent->utc();
        }
    ...
    public function toIso8601String(): string
    {
        ...
        if ($this->temporaRecurrences !== null) {
            $parts[] = 'R'.$this->temporaRecurrences;
        }
    ...
    public function toString(): string
    {
        ...
        if ($this->temporaRecurrences !== null) {
            $parts[] = $this->translate('period_recurrences', [], $this->temporaRecurrences, $translator);
        }
     ...
     * @example
     * ```
     * var_dump(TemporaPeriod::create('2021-01-05', '2021-02-15')->toDatePeriod());
     * ```
     */
     ...
     * because filters can emit `TemporaPeriod::END_ITERATION` to stop the iteration in
        ...
        foreach ($this->filters as $filter) {
            switch ($filter) {
                case [static::RECURRENCES_FILTER, null]:
                    if ($this->temporaRecurrences !== null && is_finite($this->temporaRecurrences)) {
                        return false;
                    }
    ...
    public function toArray(): array
    {
        ...
        $state = [
            $this->key,
            $this->temporaCurrent ? $this->temporaCurrent->avoidMutation() : null,
            $this->validationResult,
        ];

        $result = iterator_to_array($this);

        [$this->key, $this->temporaCurrent, $this->validationResult] = $state;
     ...
     */
    public function first(): ?TemporaInterface
    {
        if ($this->isUnfilteredAndEndLess()) {
     ...
     */
    public function last(): ?TemporaInterface
    {
        $array = $this->toArray();
     ...
     *
     * TemporaPeriod::days(3)->hours(5)->invert()
    ...
    public function __call(string $method, array $parameters): mixed
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new Tempora(), ...$parameters);
            ...
            case 'untilNow':
                return $this->setEndDate(new Tempora(), ...$parameters);
            ...
            case 'microsecond':
                return $this->setDateInterval((
                    ...
                    [$this->isDefaultInterval ? new TemporaInterval('PT0S') : $this->dateInterval, $method]
                )(...$parameters));
        }
    ...
    public function setTimezone(DateTimeZone|string|int $timezone): static
    {
        ...
        $self->timezone = TemporaTimeZone::instance($timezone);
     ...
     * @return TemporaInterface
     */
    public function calculateEnd(?string $rounding = null): TemporaInterface
    {
    ...
    private function getEndFromRecurrences(): ?TemporaInterface
    {
        if ($this->temporaRecurrences === null) {
    ...
    private function iterateUntilEnd(): ?TemporaInterface
    {
        $attempts = 0;
        $date = null;
     ...
     *
     * @param TemporaPeriod|\DateTimeInterface|Tempora|TemporaImmutable|string $rangeOrRangeStart
     * @param \DateTimeInterface|Tempora|TemporaImmutable|string|null         $rangeEnd
     *
     * @return bool
     */
     ...
     * @example
     * ```
     * Tempora::create('2020-11-29')->daysUntil('2020-12-24')->forEach(function (Tempora $date) {
     ...
     * @example
     * ```
     * $period = Tempora::create('2020-11-29')->daysUntil('2020-12-24');
     * echo implode("\n", iterator_to_array($period->map(function (Tempora $date) {
    ...
    {
        return $this->getStartDate()->lessThan($this->resolveTempora($date));
    }
    ...
    {
        return $this->getStartDate()->lessThanOrEqualTo($this->resolveTempora($date));
    }
    ...
    {
        return $this->getStartDate()->greaterThan($this->resolveTempora($date));
    }
    ...
    {
        return $this->getStartDate()->greaterThanOrEqualTo($this->resolveTempora($date));
    }
    ...
    {
        return $this->getStartDate()->equalTo($this->resolveTempora($date));
    }
    ...
    {
        return $this->calculateEnd()->lessThan($this->resolveTempora($date));
    }
    ...
    {
        return $this->calculateEnd()->lessThanOrEqualTo($this->resolveTempora($date));
    }
    ...
    {
        return $this->calculateEnd()->greaterThan($this->resolveTempora($date));
    }
    ...
    {
        return $this->calculateEnd()->greaterThanOrEqualTo($this->resolveTempora($date));
    }
    ...
    {
        return $this->calculateEnd()->equalTo($this->resolveTempora($date));
    }
    ...
    {
        $period = $this->resolveTemporaPeriod($period, ...$arguments);
    ...
    public function __unserialize(array $data): void
    {
        try {
            ...
            foreach ($values as $key => $value) {
                ...
                $property = match ($key) {
                    ...
                    'current' => function (mixed $current): void {
                        if (!($current instanceof TemporaInterface)) {
                            $current = $this->resolveTempora($current);
                        }

                        $this->temporaCurrent = $current;
                    },
                    'start' => 'startDate',
                ...
                if ($value instanceof DateTimeInterface && !($value instanceof TemporaInterface)) {
                    $value = ($value instanceof DateTime)
                        ? Tempora::instance($value)
                        : TemporaImmutable::instance($value);
                }
            ...
            if (\array_key_exists('temporaRecurrences', $values)) {
                $this->temporaRecurrences = $values['temporaRecurrences'];
            } elseif (((int) ($values['recurrences'] ?? 0)) <= 1 && $this->endDate !== null) {
                $this->temporaRecurrences = null;
            }
        } catch (Throwable $e) {
            // @codeCoverageIgnoreStart
     ...
     *
     * Will create an automatic filter callback for one of Tempora's is* methods.
     */
    protected function createFilterTuple(array $parameters): array
    {
        $method = array_shift($parameters);

        if (!$this->isTemporaPredicateMethod($method)) {
    ...
    /**
     * Return whether given callable is a string pointing to one of Tempora's is* methods
     * and should be automatically converted to a filter callback.
     */
    protected function isTemporaPredicateMethod(callable|string $callable): bool
    {
     ...
     * @SuppressWarnings(UnusedFormalParameter)
     */
    protected function filterRecurrences(TemporaInterface $current, int $key): bool|callable
    {
        if ($key < $this->temporaRecurrences) {
            return true;
        }
     ...
     */
    protected function filterEndDate(TemporaInterface $current): bool|callable
    {
    ...
    {
        if (($this->getOptions() & static::IMMUTABLE) && $this->dateClass === Tempora::class) {
            $this->dateClass = TemporaImmutable::class;
        } elseif (!($this->getOptions() & static::IMMUTABLE) && $this->dateClass === TemporaImmutable::class) {
            $this->dateClass = Tempora::class;
        }
    ...
    {
        $current = $this->prepareForReturn($this->temporaCurrent);
     ...
     *
     * @param TemporaInterface $date
     *
     * @return TemporaInterface
     */
    protected function prepareForReturn(TemporaInterface $date)
    {
    ...
    protected function incrementCurrentDateUntilValid(): void
    {
        ...
        do {
            $this->temporaCurrent = $this->temporaCurrent->add($this->dateInterval);
    ...
    /**
     * Return the Tempora instance passed through, a now instance in the same timezone
     * if null given or parse the input if string given.
     *
     * @param \LaraGram\Tempora\Tempora|\LaraGram\Tempora\TemporaPeriod|\LaraGram\Tempora\TemporaInterval|\DateInterval|\DatePeriod|\DateTimeInterface|string|null $date
     *
     * @return \LaraGram\Tempora\TemporaInterface
     */
    protected function resolveTempora($date = null)
    {
        return $this->getStartDate()->nowWithSameTz()->temporaize($date);
    }

    /**
     * Resolve passed arguments or DatePeriod to a TemporaPeriod object.
     */
    protected function resolveTemporaPeriod(mixed $period, mixed ...$arguments): self
    {
        if ($period instanceof self) {
            return $period;
        }
    ...
    {
        return $date instanceof TemporaInterface && ($date->isEndOfTime() || $date->isStartOfTime());
    }
    ...
    private function rawDate($date): ?DateTimeInterface
    {
        ...
        if ($date instanceof TemporaInterface) {
    ...
    private function initializeSerialization(array $values): void
    {
        $serializationBase = [
            ...
            'current' => $values['current'] ?? $values['temporaCurrent'] ?? null,
            'end' => $values['end'] ?? $values['endDate'] ?? null,
            'interval' => $values['interval'] ?? $values['dateInterval'] ?? null,
            'recurrences' => max(1, (int) ($values['recurrences'] ?? $values['temporaRecurrences'] ?? 1)),
        ...
        foreach (['start', 'current', 'end'] as $dateProperty) {
            if ($serializationBase[$dateProperty] instanceof Tempora) {
                $serializationBase[$dateProperty] = $serializationBase[$dateProperty]->toDateTime();
            } elseif ($serializationBase[$dateProperty] instanceof TemporaInterface) {
                $serializationBase[$dateProperty] = $serializationBase[$dateProperty]->toDateTimeImmutable();
            }
        }

        if ($serializationBase['interval'] instanceof TemporaInterval) {
