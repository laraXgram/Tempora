...
use LaraGram\Tempora\Exceptions\UnsupportedUnitException;
...
trait Units
{
     ...
     *
     * @param Unit|string|DateInterval|Closure|TemporaConverterInterface $unit
     * @param int|float                                                 $value
     * @param bool|null $overflow
     *
     * @return static
     *@example $date->add(15, 'days')
     * @example $date->add(TemporaInterval::days(4))
     *
    ...
    public function add($unit, $value = 1, ?bool $overflow = null): static
    {
        ...
        if (\is_string($unit) && \func_num_args() === 1) {
            $unit = TemporaInterval::make($unit, [], true);
        }

        if ($unit instanceof TemporaConverterInterface) {
            $unit = Closure::fromCallable([$unit, 'convertDate']);
        }

        if ($unit instanceof Closure) {
            $result = $this->resolveTempora($unit($this, false));
     ...
     * @example $date->sub(TemporaInterval::days(4))
     *
    ...
    public function sub($unit, $value = 1, ?bool $overflow = null): static
    {
        ...
        if ($unit instanceof Closure) {
            $result = $this->resolveTempora($unit($this, true));
    ...
    private static function rawAddUnit(self $date, string $unit, int|float $value): ?static
    {
        try {
            return $date->rawAdd(
                TemporaInterval::fromString(abs($value)." $unit")->invert($value < 0),
            );
