 ...
 * @method static copy()
 * @method static resolveTempora($date = null)
 */
trait Difference
{
    /**
     ...
     * current one).
     *
     * @param \LaraGram\Tempora\TemporaInterface|\DateTimeInterface|string|null $date
    ...
    /**
     * Get the difference as a TemporaInterval instance.
     * Return relative interval (negative if $absolute flag is not set to true and the given date is before
     * current one).
     *
     * @param \LaraGram\Tempora\TemporaInterface|\DateTimeInterface|string|null $date
     * @param bool                                                   $absolute Get the absolute of the difference
     *
     * @return TemporaInterval
     */
    public function diffAsTemporaInterval($date = null, bool $absolute = false, array $skip = []): TemporaInterval
    {
        return TemporaInterval::diff($this, $this->resolveTempora($date), $absolute, $skip)
            ->setLocalTranslator($this->getLocalTranslator());
    }

    /**
     * @alias diffAsTemporaInterval
     *
     * Get the difference as a DateInterval instance.
     * Return relative interval (negative if $absolute flag is not set to true and the given date is before
     * current one).
     ...
     * @return TemporaInterval
     */
    public function diff($date = null, bool $absolute = false, array $skip = []): TemporaInterval
    {
        return $this->diffAsTemporaInterval($date, $absolute, $skip);
    }
    ...
    public function diffInYears($date = null, bool $absolute = false, bool $utc = false): float
    {
        ...
        /** @var Tempora|TemporaImmutable $floorEnd */
        ...
        /** @var Tempora|TemporaImmutable $ceilEnd */
    ...
    {
        return $this->diffFiltered(TemporaInterval::day(), $callback, $date, $absolute);
    }
    ...
    {
        return $this->diffFiltered(TemporaInterval::hour(), $callback, $date, $absolute);
    }

    /**
     * Get the difference by the given interval using a filter closure.
     *
     * @param TemporaInterval                                         $ci       An interval to traverse by
     * @param Closure                                                $callback
     * @param \LaraGram\Tempora\TemporaInterface|\DateTimeInterface|string|null $date
     * @param bool                                                   $absolute Get the absolute of the difference
     *
     * @return int
     */
    public function diffFiltered(TemporaInterval $ci, Closure $callback, $date = null, bool $absolute = false): int
    {
        $start = $this;
        $end = $this->resolveTempora($date);
        $inverse = false;
        ...
        $options = TemporaPeriod::EXCLUDE_END_DATE | ($this->isMutable() ? 0 : TemporaPeriod::IMMUTABLE);
    ...
    public function diffInWeekdays($date = null, bool $absolute = false): int
    {
        return $this->diffInDaysFiltered(
            static fn (TemporaInterface $date) => $date->isWeekday(),
            $this->resolveTempora($date)->avoidMutation()->modify($this->format('H:i:s.u')),
            $absolute,
        );
    }
    ...
    {
        return $this->diffInDaysFiltered(
            static fn (TemporaInterface $date) => $date->isWeekend(),
            $this->resolveTempora($date)->avoidMutation()->modify($this->format('H:i:s.u')),
            $absolute,
        );
    }
     ...
     * @example
     * ```
     * echo Tempora::tomorrow()->diffForHumans() . "\n";
     * echo Tempora::tomorrow()->diffForHumans(['parts' => 2]) . "\n";
     * echo Tempora::tomorrow()->diffForHumans(['parts' => 3, 'join' => true]) . "\n";
     * echo Tempora::tomorrow()->diffForHumans(Tempora::yesterday()) . "\n";
     * echo Tempora::tomorrow()->diffForHumans(Tempora::yesterday(), ['short' => true]) . "\n";
     * ```
     *
     * @param Tempora|DateTimeInterface|string|array|null $other   if array passed, will be used as parameters array, see $syntax below;
    ...
    {
        if (!$syntax && !$other) {
            $syntax = TemporaInterface::DIFF_RELATIVE_TO_NOW;
        }

        return $this->resolveTempora($other)->diffForHumans($this, $syntax, $short, $parts, $options);
    }
     ...
     *
     * @param Tempora|\DateTimeInterface|string|null $referenceTime
     * @param array                                 $formats
     *
     * @return string
     */
    ...
    {
        /** @var TemporaInterface $current */
        $current = $this->avoidMutation()->startOfDay();
        /** @var TemporaInterface $other */
        $other = $this->resolveTempora($referenceTime)->avoidMutation()->setTimezone($this->getTimezone())->startOfDay();
