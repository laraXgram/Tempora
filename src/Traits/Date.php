...
use LaraGram\Tempora\NativeClock;
use Throwable;
...
trait Date
{
    ...
    protected static $days = [
        // @call isDayOfWeek
        TemporaInterface::SUNDAY => 'Sunday',
        // @call isDayOfWeek
        TemporaInterface::MONDAY => 'Monday',
        // @call isDayOfWeek
        TemporaInterface::TUESDAY => 'Tuesday',
        // @call isDayOfWeek
        TemporaInterface::WEDNESDAY => 'Wednesday',
        // @call isDayOfWeek
        TemporaInterface::THURSDAY => 'Thursday',
        // @call isDayOfWeek
        TemporaInterface::FRIDAY => 'Friday',
        // @call isDayOfWeek
        TemporaInterface::SATURDAY => 'Saturday',
    ];
    ...
    protected static function safeCreateDateTimeZone(
        ...
        return TemporaTimeZone::instance($object, $objectDump);
    }

    /**
     * Get the TimeZone associated with the Tempora instance (as TemporaTimeZone).
     *
     * @link https://php.net/manual/en/datetime.gettimezone.php
     */
    public function getTimezone(): TemporaTimeZone
    {
        return $this->transmitFactory(fn () => TemporaTimeZone::instance(parent::getTimezone()));
    }
    ...
    public function temporaize($date = null)
    {
        ...
        if ($date instanceof DatePeriod || $date instanceof TemporaPeriod) {
            $date = $date->getStartDate();
        }

        return $this->resolveTempora($date);
    }
    ...
    public function get(Unit|string $name): mixed
    {
        ...
        switch (true) {
            ...
            // @property-read string long name of weekday translated according to Tempora locale, in english if no translation available for current language
            case $name === 'dayName':
                return $this->getTranslatedDayName();
            // @property-read string short name of weekday translated according to Tempora locale, in english if no translation available for current language
            case $name === 'shortDayName':
                return $this->getTranslatedShortDayName();
            // @property-read string very short name of weekday translated according to Tempora locale, in english if no translation available for current language
            case $name === 'minDayName':
                return $this->getTranslatedMinDayName();
            // @property-read string long name of month translated according to Tempora locale, in english if no translation available for current language
            case $name === 'monthName':
                return $this->getTranslatedMonthName();
            // @property-read string short name of month translated according to Tempora locale, in english if no translation available for current language
            case $name === 'shortMonthName':
                return $this->getTranslatedShortMonthName();
            // @property-read string lowercase meridiem mark translated according to Tempora locale, in latin if no translation available for current language
            case $name === 'meridiem':
                return $this->meridiem(true);
            // @property-read string uppercase meridiem mark translated according to Tempora locale, in latin if no translation available for current language
            ...
            // @--property-read TemporaTimeZone $timezone the current timezone
            // @--property-read TemporaTimeZone $tz alias of $timezone
            // @property TemporaTimeZone $timezone the current timezone
            // @property TemporaTimeZone $tz alias of $timezone
    ...
    public function utcOffset(?int $minuteOffset = null): static|int
    {
        ...
        return $this->setTimezone(TemporaTimeZone::createFromMinuteOffset($minuteOffset));
    }
    ...
    public static function getIsoUnits(): array
    {
        ...
        $units ??= [
            ...
            'Do' => ['ordinal', ['day', 'D']],
            'd' => 'dayOfWeek',
            'dd' => static fn (TemporaInterface $date, $originalFormat = null) => $date->getTranslatedMinDayName(
                $originalFormat,
            ),
            'ddd' => static fn (TemporaInterface $date, $originalFormat = null) => $date->getTranslatedShortDayName(
                $originalFormat,
            ),
            'dddd' => static fn (TemporaInterface $date, $originalFormat = null) => $date->getTranslatedDayName(
                $originalFormat,
            ),
            'DDD' => 'dayOfYear',
            ...
            'S' => static fn (TemporaInterface $date) => (string) floor($date->micro / 100000),
            'SS' => static fn (TemporaInterface $date) => self::floorZeroPad($date->micro / 10000, 2),
            'SSS' => static fn (TemporaInterface $date) => self::floorZeroPad($date->micro / 1000, 3),
            'SSSS' => static fn (TemporaInterface $date) => self::floorZeroPad($date->micro / 100, 4),
            'SSSSS' => static fn (TemporaInterface $date) => self::floorZeroPad($date->micro / 10, 5),
            'SSSSSS' => ['getPaddedUnit', ['micro', 6]],
            'SSSSSSS' => static fn (TemporaInterface $date) => self::floorZeroPad($date->micro * 10, 7),
            'SSSSSSSS' => static fn (TemporaInterface $date) => self::floorZeroPad($date->micro * 100, 8),
            'SSSSSSSSS' => static fn (TemporaInterface $date) => self::floorZeroPad($date->micro * 1000, 9),
            'M' => 'month',
            'MM' => ['rawFormat', ['m']],
            'MMM' => static function (TemporaInterface $date, $originalFormat = null) {
            ...
            'MMMM' => static fn (TemporaInterface $date, $originalFormat = null) => $date->getTranslatedMonthName(
                $originalFormat,
            ),
            'Mo' => ['ordinal', ['month', 'M']],
            'Q' => 'quarter',
            ...
            'YYYYYY' => static fn (TemporaInterface $date) => ($date->year < 0 ? '' : '+').
    ...
    public function isoFormat(string $format, ?string $originalFormat = null): string
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if (preg_match('/^'.TemporaInterface::ISO_FORMAT_REGEXP.'/', $input, $match)) {
                $code = $match[0];
     ...
     * if null given or parse the input if string given.
     */
    protected function resolveTempora(DateTimeInterface|string|null $date): self
    {
        if (!$date) {
    ...
    private function callHumanDiffAlias(string $method, array $parameters): ?string
    {
        ...
        $diffSyntaxModes = [
            // @call diffForHumans
            'Absolute' => TemporaInterface::DIFF_ABSOLUTE,
            // @call diffForHumans
            'Relative' => TemporaInterface::DIFF_RELATIVE_AUTO,
            // @call diffForHumans
            'RelativeToNow' => TemporaInterface::DIFF_RELATIVE_TO_NOW,
            // @call diffForHumans
            'RelativeToOther' => TemporaInterface::DIFF_RELATIVE_TO_OTHER,
        ];
    ...
    private function callPeriodMethod(string $method, array $parameters): ?TemporaPeriod
    {
     ...
     * @return T
     */
    private function mutateIfMutable(TemporaInterface $date): TemporaInterface
    {
