...
use LaraGram\Tempora\Exceptions\InvalidTypeException;
use LaraGram\Tempora\Exceptions\NotLocaleAwareException;
use LaraGram\Tempora\Language;
use LaraGram\Tempora\Translator;
use LaraGram\Tempora\TranslatorStrongTypeInterface;
use Closure;
use LaraGram\Tempora\Translation\TranslatorBagInterface;
use LaraGram\Tempora\Translation\LocaleAwareInterface;
...
trait Localization
{
     ...
     * @param string|null $from       input locale of the $timeString parameter (`Tempora::getLocale()` by default)
     * @param string|null $to         output locale of the result returned (`"en"` by default)
     * @param int         $mode       specify what to translate with options:
     *                                - TemporaInterface::TRANSLATE_ALL (default)
     *                                - TemporaInterface::TRANSLATE_MONTHS
     *                                - TemporaInterface::TRANSLATE_DAYS
     *                                - TemporaInterface::TRANSLATE_UNITS
     *                                - TemporaInterface::TRANSLATE_MERIDIEM
     *                                You can use pipe to group: TemporaInterface::TRANSLATE_MONTHS | TemporaInterface::TRANSLATE_DAYS
     *
     * @return string
     */
    public static function translateTimeString(
        ...
        ?string $from = null,
        ?string $to = null,
        int $mode = TemporaInterface::TRANSLATE_ALL,
    ): string {
        // Fallback source and destination locales
        $from = $from ?: static::getLocale();
        $to = $to ?: TemporaInterface::DEFAULT_LOCALE;
        ...
        foreach (['from', 'to'] as $key) {
            ...
            $$translationKey = array_merge(
                $mode & TemporaInterface::TRANSLATE_MONTHS ? static::getTranslationArray($months, static::MONTHS_PER_YEAR, $timeString) : [],
                $mode & TemporaInterface::TRANSLATE_MONTHS ? static::getTranslationArray($messages['months_short'] ?? [], static::MONTHS_PER_YEAR, $timeString) : [],
                $mode & TemporaInterface::TRANSLATE_DAYS ? static::getTranslationArray($weekdays, static::DAYS_PER_WEEK, $timeString) : [],
                $mode & TemporaInterface::TRANSLATE_DAYS ? static::getTranslationArray($messages['weekdays_short'] ?? [], static::DAYS_PER_WEEK, $timeString) : [],
                $mode & TemporaInterface::TRANSLATE_DIFF ? static::translateWordsByKeys([
                    'diff_now',
                    'diff_today',
                    'diff_yesterday',
                    'diff_tomorrow',
                ...
                $mode & TemporaInterface::TRANSLATE_UNITS ? static::translateWordsByKeys([
                    'year',
                    'month',
                    'week',
                    'day',
                ...
                $mode & TemporaInterface::TRANSLATE_MERIDIEM ? array_map(function ($hour) use ($meridiem) {
     ...
     * @param \LaraGram\Tempora\Translation\MessageCatalogueInterface $catalogue
     *
     * @return mixed
     */
