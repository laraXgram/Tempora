...
use LaraGram\Tempora\Exceptions\BadFluentConstructorException;
use LaraGram\Tempora\Exceptions\BadFluentSetterException;
use LaraGram\Tempora\Exceptions\InvalidCastException;
use LaraGram\Tempora\Exceptions\InvalidFormatException;
use LaraGram\Tempora\Exceptions\InvalidIntervalException;
use LaraGram\Tempora\Exceptions\OutOfRangeException;
use LaraGram\Tempora\Exceptions\ParseErrorException;
use LaraGram\Tempora\Exceptions\UnitNotConfiguredException;
use LaraGram\Tempora\Exceptions\UnknownGetterException;
use LaraGram\Tempora\Exceptions\UnknownSetterException;
use LaraGram\Tempora\Exceptions\UnknownUnitException;
use LaraGram\Tempora\Traits\IntervalRounding;
use LaraGram\Tempora\Traits\IntervalStep;
use LaraGram\Tempora\Traits\LocalFactory;
use LaraGram\Tempora\Traits\MagicParameter;
use LaraGram\Tempora\Traits\Mixin;
use LaraGram\Tempora\Traits\Options;
use LaraGram\Tempora\Traits\ToStringFormat;
use Closure;
use DateInterval;
use DateTime;
 ...
 * @property int $seconds Second component of the current interval. (TemporaInterval::minutes(2)->seconds(34)->microseconds(567_890)->seconds = 34)
 * @property int $milliseconds Milliseconds component of the current interval. (TemporaInterval::seconds(34)->microseconds(567_890)->milliseconds = 567)
 * @property int $microseconds Microseconds component of the current interval. (TemporaInterval::seconds(34)->microseconds(567_890)->microseconds = 567_890)
 ...
 * @property-read float $totalSeconds Number of seconds equivalent to the interval. (TemporaInterval::minutes(2)->seconds(34)->microseconds(567_890)->totalSeconds = 154.567_890)
 * @property-read float $totalMilliseconds Number of milliseconds equivalent to the interval. (TemporaInterval::seconds(34)->microseconds(567_890)->totalMilliseconds = 34567.890)
 * @property-read float $totalMicroseconds Number of microseconds equivalent to the interval. (TemporaInterval::seconds(34)->microseconds(567_890)->totalMicroseconds = 34567890)
 * @property-read string $locale locale of the current instance
 *
 * @method static TemporaInterval years($years = 1) Create instance specifying a number of years or modify the number of years if called on an instance.
 * @method static TemporaInterval year($years = 1) Alias for years()
 * @method static TemporaInterval months($months = 1) Create instance specifying a number of months or modify the number of months if called on an instance.
 * @method static TemporaInterval month($months = 1) Alias for months()
 * @method static TemporaInterval weeks($weeks = 1) Create instance specifying a number of weeks or modify the number of weeks if called on an instance.
 * @method static TemporaInterval week($weeks = 1) Alias for weeks()
 * @method static TemporaInterval days($days = 1) Create instance specifying a number of days or modify the number of days if called on an instance.
 * @method static TemporaInterval dayz($days = 1) Alias for days()
 * @method static TemporaInterval daysExcludeWeeks($days = 1) Create instance specifying a number of days or modify the number of days (keeping the current number of weeks) if called on an instance.
 * @method static TemporaInterval dayzExcludeWeeks($days = 1) Alias for daysExcludeWeeks()
 * @method static TemporaInterval day($days = 1) Alias for days()
 * @method static TemporaInterval hours($hours = 1) Create instance specifying a number of hours or modify the number of hours if called on an instance.
 * @method static TemporaInterval hour($hours = 1) Alias for hours()
 * @method static TemporaInterval minutes($minutes = 1) Create instance specifying a number of minutes or modify the number of minutes if called on an instance.
 * @method static TemporaInterval minute($minutes = 1) Alias for minutes()
 * @method static TemporaInterval seconds($seconds = 1) Create instance specifying a number of seconds or modify the number of seconds if called on an instance.
 * @method static TemporaInterval second($seconds = 1) Alias for seconds()
 * @method static TemporaInterval milliseconds($milliseconds = 1) Create instance specifying a number of milliseconds or modify the number of milliseconds if called on an instance.
 * @method static TemporaInterval millisecond($milliseconds = 1) Alias for milliseconds()
 * @method static TemporaInterval microseconds($microseconds = 1) Create instance specifying a number of microseconds or modify the number of microseconds if called on an instance.
 * @method static TemporaInterval microsecond($microseconds = 1) Alias for microseconds()
 ...
 */
class TemporaInterval extends DateInterval implements TemporaConverterInterface
{
    use LocalFactory;
    ...
    public const SPECIAL_TRANSLATIONS = [
        1 => [
            'option' => TemporaInterface::ONE_DAY_WORDS,
            'future' => 'diff_tomorrow',
            'past' => 'diff_yesterday',
        ],
        2 => [
            'option' => TemporaInterface::TWO_DAY_WORDS,
     ...
     */
    protected ?TemporaInterface $startDate = null;

    /**
     * End date if interval was created from a difference between 2 dates.
     */
    protected ?TemporaInterface $endDate = null;
    ...
    {
        return [
            'milliseconds' => [TemporaInterface::MICROSECONDS_PER_MILLISECOND, 'microseconds'],
            'seconds' => [TemporaInterface::MILLISECONDS_PER_SECOND, 'milliseconds'],
            'minutes' => [TemporaInterface::SECONDS_PER_MINUTE, 'seconds'],
            'hours' => [TemporaInterface::MINUTES_PER_HOUR, 'minutes'],
            'dayz' => [TemporaInterface::HOURS_PER_DAY, 'hours'],
            'weeks' => [TemporaInterface::DAYS_PER_WEEK, 'dayz'],
            'months' => [TemporaInterface::WEEKS_PER_MONTH, 'weeks'],
            'years' => [TemporaInterface::MONTHS_PER_YEAR, 'months'],
        ];
    }
    ...
    /**
     * This option allow you to opt-in for the Tempora 3 behavior where float
     ...
     * code including the third-party dependencies that also may use LaraGram\Tempora will
    ...
    /**
     * Create a new TemporaInterval instance.
     *
    ...
    public function __construct($years = null, $months = null, $weeks = null, $days = null, $hours = null, $minutes = null, $seconds = null, $microseconds = null)
    {
        ...
        try {
            ...
            try {
                ...
                $this->i = (int) $minutes;
                $this->s = (int) $seconds;
                $secondFloatPart = (float) ($microseconds / TemporaInterface::MICROSECONDS_PER_SECOND);
                $this->f = $secondFloatPart;
                $intervalMicroseconds = (int) ($this->f * TemporaInterface::MICROSECONDS_PER_SECOND);
        ...
        if ($microseconds !== null) {
            $this->f = $microseconds / TemporaInterface::MICROSECONDS_PER_SECOND;
        }
     ...
     * else try to find the appropriate constant as the factor, such as Tempora::DAYS_PER_WEEK.
     *
     * @param string $source
     * @param string $target
     *
    ...
    public static function getFactorWithDefault($source, $target)
    {
        ...
        static $defaults = [
            'month' => ['year' => Tempora::MONTHS_PER_YEAR],
            'week' => ['month' => Tempora::WEEKS_PER_MONTH],
            'day' => ['week' => Tempora::DAYS_PER_WEEK],
            'hour' => ['day' => Tempora::HOURS_PER_DAY],
            'minute' => ['hour' => Tempora::MINUTES_PER_HOUR],
            'second' => ['minute' => Tempora::SECONDS_PER_MINUTE],
            'millisecond' => ['second' => Tempora::MILLISECONDS_PER_SECOND],
            'microsecond' => ['millisecond' => Tempora::MICROSECONDS_PER_MILLISECOND],
        ];
    ...
    {
        return static::getFactor('dayz', 'weeks') ?: Tempora::DAYS_PER_WEEK;
    }
    ...
    {
        return static::getFactor('hours', 'dayz') ?: Tempora::HOURS_PER_DAY;
    }
    ...
    {
        return static::getFactor('minutes', 'hours') ?: Tempora::MINUTES_PER_HOUR;
    }
    ...
    {
        return static::getFactor('seconds', 'minutes') ?: Tempora::SECONDS_PER_MINUTE;
    }
    ...
    {
        return static::getFactor('milliseconds', 'seconds') ?: Tempora::MILLISECONDS_PER_SECOND;
    }
    ...
    {
        return static::getFactor('microseconds', 'milliseconds') ?: Tempora::MICROSECONDS_PER_MILLISECOND;
    }

    /**
     * Create a new TemporaInterval instance from specific values.
     * This is an alias for the constructor that allows better fluent
     * syntax as it allows you to do TemporaInterval::create(1)->fn() rather than
     * (new TemporaInterval(1))->fn().
     *
     * @param int $years
     * @param int $months
     * @param int $weeks
    ...
    /**
     * Parse a string into a new TemporaInterval object according to the specified format.
     *
     ...
     *
     * @return static
     *@throws \LaraGram\Tempora\Exceptions\ParseErrorException when the $interval cannot be parsed as an interval.
     *
     * @example
     * ```
     * echo Temporainterval::createFromFormat('H:i', '1:30');
     * ```
     *
     */
     ...
     *
     * @return TemporaInterface|null
     */
    public function start(): ?TemporaInterface
    {
        $this->checkStartAndEnd();
     ...
     * @return TemporaInterface|null
     */
    public function end(): ?TemporaInterface
    {
        $this->checkStartAndEnd();
    ...
    /**
     * Provide static helpers to create instances.  Allows TemporaInterval::years(3).
     *
    ...
    public static function __callStatic(string $method, array $parameters)
    {
        try {
            ...
            if (Tempora::isStrictModeEnabled()) {
    ...
    /**
     * Evaluate the PHP generated by var_export() and recreate the exported TemporaInterval instance.
     *
    ...
    /**
     * Creates a TemporaInterval from string.
     *
     * Format:
     *
    ...
    public static function fromString(string $intervalDefinition): static
    {
        ...
        while ([$part, $value, $unit] = array_shift($parts)) {
            ...
            switch ($unit === 'µs' ? 'µs' : strtolower($unit)) {
                case 'millennia':
                case 'millennium':
                    $years += $intValue * TemporaInterface::YEARS_PER_MILLENNIUM;

                    break;

                case 'century':
                case 'centuries':
                    $years += $intValue * TemporaInterface::YEARS_PER_CENTURY;

                    break;

                case 'decade':
                case 'decades':
                    $years += $intValue * TemporaInterface::YEARS_PER_DECADE;
                ...
                case 'quarter':
                case 'quarters':
                    $months += $intValue * TemporaInterface::MONTHS_PER_QUARTER;
        ...
        return self::withOriginal(
            new static($years, $months, $weeks, $days, $hours, $minutes, $seconds, $milliseconds * Tempora::MICROSECONDS_PER_MILLISECOND + $microseconds),
            $intervalDefinition,
        );
    }

    /**
     * Creates a TemporaInterval from string using a different locale.
     *
    ...
    {
        return static::fromString(Tempora::translateTimeString($interval, $locale ?: static::getLocale(), TemporaInterface::DEFAULT_LOCALE));
    }

    /**
     * Create an interval from the difference between 2 dates.
     *
     * @param \LaraGram\Tempora\Tempora|\DateTimeInterface|mixed $start
     * @param \LaraGram\Tempora\Tempora|\DateTimeInterface|mixed $end
     *
     * @return static
     */
    public static function diff($start, $end = null, bool $absolute = false, array $skip = []): static
    {
        $start = $start instanceof TemporaInterface ? $start : Tempora::make($start);
        $end = $end instanceof TemporaInterface ? $end : Tempora::make($end);
    ...
    /**
     * Create a TemporaInterval instance from a DateInterval one.  Can not instance
    ...
    /**
     * Make a TemporaInterval instance from given variable if possible.
     *
    ...
    /**
     * Get a part of the TemporaInterval object.
     */
    public function get(Unit|string $name): int|float|string|null
    {
        ...
        $resolvedUnit = Tempora::singularUnit(rtrim($name, 'z'));
        ...
        return match ($resolvedUnit) {
            'tzname', 'tz_name' => match (true) {
                ...
                default => TemporaTimeZone::instance($this->timezoneSetting)->getName(),
            },
            'year' => $this->y,
            'month' => $this->m,
            'day' => $this->d,
            ...
            'minute' => $this->i,
            'second' => $this->s,
            'milli', 'millisecond' => (int) (round($this->f * Tempora::MICROSECONDS_PER_SECOND) /
                Tempora::MICROSECONDS_PER_MILLISECOND),
            'micro', 'microsecond' => (int) round($this->f * Tempora::MICROSECONDS_PER_SECOND),
            'microexcludemilli' => (int) round($this->f * Tempora::MICROSECONDS_PER_SECOND) %
                Tempora::MICROSECONDS_PER_MILLISECOND,
    ...
    /**
     * Set a part of the TemporaInterval object.
     *
    ...
    public function set($name, $value = null): static
    {
        ...
        foreach ($properties as $key => $value) {
            switch (Tempora::singularUnit($key instanceof Unit ? $key->value : rtrim((string) $key, 'z'))) {
                case 'year':
                ...
                case 'milli':
                case 'millisecond':
                    $this->microseconds = $value * Tempora::MICROSECONDS_PER_MILLISECOND + $this->microseconds % Tempora::MICROSECONDS_PER_MILLISECOND;

                    break;

                case 'micro':
                case 'microsecond':
                    $this->f = $value / Tempora::MICROSECONDS_PER_SECOND;
                ...
                default:
                    ...
                    if ($this->localStrictModeEnabled ?? Tempora::isStrictModeEnabled()) {
     ...
     * @example
     * ```
     * TemporaInterval::macro('twice', function () {
     *   return $this->times(2);
     * });
     * echo TemporaInterval::hours(2)->twice();
     * ```
     *
     ...
     * @example
     * ```
     * TemporaInterval::mixin(new class {
     ...
     *   }
     * });
     * echo TemporaInterval::hours(5)->hoursToDays() . "\n";
     * echo TemporaInterval::days(5)->daysToHours() . "\n";
     * ```
     *
    ...
    /**
     * Allow fluent calls on the setters... TemporaInterval::years(3)->months(5)->day().
     *
     * Note: This is done using the magic method to allow static and instance methods to
     *       have the same names.
     *
    ...
    public function __call(string $method, array $parameters)
    {
        ...
        if (preg_match('/^(?<method>add|sub)(?<unit>[A-Z].*)$/', $method, $match)) {
            $value = $this->getMagicParameter($parameters, 0, Tempora::pluralUnit($match['unit']), 0);

            return $this->{$match['method']}($value, $match['unit']);
        }

        $value = $this->getMagicParameter($parameters, 0, Tempora::pluralUnit($method), 1);
    ...
    protected function getForHumansInitialVariables($syntax, $short): array
    {
        ...
        if (\is_bool($syntax)) {
            return [
                'short' => $syntax,
                'syntax' => TemporaInterface::DIFF_ABSOLUTE,
            ];
        }
    ...
    {
        if ($options & TemporaInterface::ROUND) {
            return 'round';
        }

        if ($options & TemporaInterface::CEIL) {
            return 'ceil';
        }

        if ($options & TemporaInterface::FLOOR) {
            return 'floor';
        }
     ...
     * @example
     * ```
     * echo TemporaInterval::fromString('4d 3h 40m')->forHumans() . "\n";
     * echo TemporaInterval::fromString('4d 3h 40m')->forHumans(['parts' => 2]) . "\n";
     * echo TemporaInterval::fromString('4d 3h 40m')->forHumans(['parts' => 3, 'join' => true]) . "\n";
     * echo TemporaInterval::fromString('4d 3h 40m')->forHumans(['short' => true]) . "\n";
     * echo TemporaInterval::fromString('1d 24h')->forHumans(['join' => ' or ']) . "\n";
     * echo TemporaInterval::fromString('1d 24h')->forHumans(['minimumUnit' => 'hour']) . "\n";
     * ```
     *
     ...
     *                           Possible values:
     *                           - TemporaInterface::DIFF_ABSOLUTE          no modifiers
     *                           - TemporaInterface::DIFF_RELATIVE_TO_NOW   add ago/from now modifier
     *                           - TemporaInterface::DIFF_RELATIVE_TO_OTHER add before/after modifier
     *                           Default value: TemporaInterface::DIFF_ABSOLUTE
    ...
    public function forHumans($syntax = null, $short = false, $parts = self::NO_LIMIT, $options = null): string
    {
        ...
        $syntax = (int) ($syntax ?? TemporaInterface::DIFF_ABSOLUTE);
        $absolute = $syntax === TemporaInterface::DIFF_ABSOLUTE;
        $relativeToNow = $syntax === TemporaInterface::DIFF_RELATIVE_TO_NOW;
        $count = 1;
        $unit = $short ? 's' : 'second';
        ...
        foreach ($diffIntervalArray as $diffIntervalData) {
            ...
            } elseif ($options & TemporaInterface::SEQUENTIAL_PARTS_ONLY && \count($interval) > 0) {
                break;
            }
        ...
        if (\count($interval) === 0) {
            if ($relativeToNow && $options & TemporaInterface::JUST_NOW) {
                $key = 'diff_now';
            ...
            $count = $options & TemporaInterface::NO_ZERO_DIFF ? 1 : 0;
     ...
     * @example
     * ```
     * var_dump(TemporaInterval::hours(2)->toDateInterval());
     * ```
     *
     * @return DateInterval
     */
    ...
    public function toPeriod(...$params): TemporaPeriod
    {
        ...
        $class = ($params[0] ?? null) instanceof DateTime ? TemporaPeriod::class : TemporaPeriodImmutable::class;
     ...
     * @return TemporaPeriod
     */
    public function stepBy($interval, Unit|string|null $unit = null): TemporaPeriod
    {
        $this->checkStartAndEnd();
        $start = $this->startDate ?? TemporaImmutable::make('now');
        ...
        $class = $start instanceof DateTime ? TemporaPeriod::class : TemporaPeriodImmutable::class;
    ...
    public static function getDateIntervalSpec(DateInterval $interval, bool $microseconds = false, array $skip = []): string
    {
        ...
        if (
            $interval->days >= TemporaInterface::DAYS_PER_WEEK * TemporaInterface::WEEKS_PER_MONTH &&
    ...
    public function total(string $unit): float
    {
        ...
        $values = [
            ...
            'minutes' => $this->minutes,
            'seconds' => $this->seconds,
            'milliseconds' => (int) ($this->microseconds / Tempora::MICROSECONDS_PER_MILLISECOND),
            'microseconds' => $this->microseconds % Tempora::MICROSECONDS_PER_MILLISECOND,
        ];
     ...
     *
     * @param TemporaInterval|DateInterval|mixed $interval
     *
     * @return bool
     *@see equalTo()
     *
     ...
     *
     * @param TemporaInterval|DateInterval|mixed $interval1
     * @param TemporaInterval|DateInterval|mixed $interval2
     * @param bool                              $equal     Indicates if an equal to comparison should be done
     *
     * @return bool
     *@example
     * ```
     * TemporaInterval::hours(48)->between(TemporaInterval::day(), TemporaInterval::days(3)); // true
     * TemporaInterval::hours(48)->between(TemporaInterval::day(), TemporaInterval::hours(36)); // false
     * TemporaInterval::hours(48)->between(TemporaInterval::day(), TemporaInterval::days(2)); // true
     * TemporaInterval::hours(48)->between(TemporaInterval::day(), TemporaInterval::days(2), false); // false
     * ```
     *
     */
     ...
     *@example
     * ```
     * TemporaInterval::hours(48)->betweenExcluded(TemporaInterval::day(), TemporaInterval::days(3)); // true
     * TemporaInterval::hours(48)->betweenExcluded(TemporaInterval::day(), TemporaInterval::hours(36)); // false
     * TemporaInterval::hours(48)->betweenExcluded(TemporaInterval::day(), TemporaInterval::days(2)); // true
     * ```
     *
     */
     ...
     * TemporaInterval::hours(48)->betweenExcluded(TemporaInterval::day(), TemporaInterval::days(3)); // true
     * TemporaInterval::hours(48)->betweenExcluded(TemporaInterval::day(), TemporaInterval::hours(36)); // false
     * TemporaInterval::hours(48)->betweenExcluded(TemporaInterval::day(), TemporaInterval::days(2)); // false
     * ```
     *
     */
     ...
     *@example
     * ```
     * TemporaInterval::hours(48)->isBetween(TemporaInterval::day(), TemporaInterval::days(3)); // true
     * TemporaInterval::hours(48)->isBetween(TemporaInterval::day(), TemporaInterval::hours(36)); // false
     * TemporaInterval::hours(48)->isBetween(TemporaInterval::day(), TemporaInterval::days(2)); // true
     * TemporaInterval::hours(48)->isBetween(TemporaInterval::day(), TemporaInterval::days(2), false); // false
     * ```
     *
     */
    ...
    public function roundUnit(string $unit, DateInterval|string|int|float $precision = 1, string $function = 'round'): static
    {
        ...
        $base = TemporaImmutable::parse('2000-01-01 00:00:00', 'UTC')
     ...
     * https://github.com/briannesbitt/Tempora/issues/3018#issuecomment-2888538438
     *
    ...
    private static function buildSerializationString(
        ...
        // De-enhance TemporaInterval objects to be serializable back to DateInterval
        if ($interval instanceof self && !is_a($className, self::class, true)) {
    ...
    private function checkIntegerValue(string $name, mixed $value): void
    {
        ...
        if (!self::$floatSettersEnabled) {
            ...
            @trigger_error(
                ...
                "- To adopt the new behavior globally, call TemporaInterval::enableFloatSetters()\n",
                \E_USER_DEPRECATED,
            );
        }
    }
