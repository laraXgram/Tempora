...
class TemporaTimeZone extends DateTimeZone
{
    use LocalFactory;
    ...
    /**
     * Create a TemporaTimeZone from mixed input.
     *
     * @param DateTimeZone|string|int|false|null $object     original value to get TemporaTimeZone from it.
    ...
    public function toOffsetName(?DateTimeInterface $date = null): string
    {
        return static::getOffsetNameFromMinuteOffset(
            $this->getOffset($this->resolveTempora($date)) / 60,
        );
    }

    /**
     * Returns a new TemporaTimeZone object using the offset string instead of region string.
     */
    ...
    public function toRegionName(?DateTimeInterface $date = null, int $isDST = 1): ?string
    {
        ...
        foreach (timezone_identifiers_list() as $timezone) {
            if (Tempora::instance($date)->setTimezone($timezone)->getOffset() === $offset) {
                return $timezone;
            }
        }
    ...
    /**
     * Returns a new TemporaTimeZone object using the region string instead of offset string.
     */
    public function toRegionTimeZone(?DateTimeInterface $date = null): ?self
    {
        ...
        if (Tempora::isStrictModeEnabled()) {
            throw new InvalidTimeZoneException('Unknown timezone for offset '.$this->getOffset($this->resolveTempora($date)).' seconds.');
        }
    ...
    /**
     * Create a TemporaTimeZone from int/float hour offset.
     *
    ...
    {
        return static::createFromMinuteOffset($hourOffset * Tempora::MINUTES_PER_HOUR);
    }

    /**
     * Create a TemporaTimeZone from int/float minute offset.
     *
    ...
    private function resolveTempora(?DateTimeInterface $date): DateTimeInterface
    {
        if ($date) {
            return $date;
        }
